...
from services.similar import create_similar_content_engine, SimilarContentEngine
import re
...
try:
    with app.app_context():
        ...
        similar_engine = create_similar_content_engine(db, models, cache)
        logger.info("Details, Content and Similar services initialized successfully")
except Exception as e:
...
def get_similar_recommendations(content_id):
    try:
        ...
        language_strict = request.args.get('language_strict', 'true').lower() == 'true'
        min_similarity = float(request.args.get('min_similarity', 0.3))
        include_metadata = request.args.get('include_metadata', 'true').lower() == 'true'
        
        if similar_engine:
            results = similar_engine.get_similar_content(
            ...
            logger.warning("Similar Content Engine not available, using fallback")
            return get_similar_recommendations_fallback(content_id, limit)
...
def get_similar_recommendations_fallback(content_id, limit):
    try:
        content = Content.query.get(content_id)
        if not content:
            return jsonify({'error': 'Content not found', 'similar_content': []}), 404
        ...
        return jsonify({
            ...
            'similar_content': [],
            'metadata': {'algorithm': 'fallback', 'message': 'No similar content found'}
        }), 200
        
    except Exception as e:
        logger.error(f"Fallback similar recommendations error: {e}")
        return jsonify({'error': 'Fallback failed', 'similar_content': []}), 500
...
def explore_genre(genre):
    try:
        ...
        if similar_engine:
            results = similar_engine.explore_genre(
                genre=genre,
            ...
            return explore_genre_fallback(genre, content_type, language_filter, limit, sort_by)
...
def explore_genre_fallback(genre, content_type, language_filter, limit, sort_by):
    try:
        ...
        if language_filter:
            query = query.filter(Content.languages.contains(language_filter))
        
        if sort_by == 'rating':
            query = query.order_by(desc(Content.rating))
        elif sort_by == 'year':
            query = query.order_by(desc(Content.release_date))
        else:
        ...
        return jsonify({
            ...
            'metadata': {
                'algorithm': 'fallback_genre_exploration',
        ...
        logger.error(f"Fallback genre exploration error: {e}")
        return jsonify({
            'error': 'Fallback genre exploration failed',
            'genre': genre,
            'content': []
        }), 500

@app.route('/api/recommendations/genre-personalized', methods=['GET'])
@auth_required
def get_personalized_genre_recommendations():
    try:
        user_id = request.user_id
        limit = min(int(request.args.get('limit', 20)), 50)
        discovery_mode = request.args.get('discovery_mode', 'false').lower() == 'true'
        
        user_preferences = get_user_preferences(user_id)
        
        if similar_engine:
            results = similar_engine.get_genre_recommendations(
            ...
            return jsonify({
                'error': 'Genre recommendation engine not available',
                'recommendations': []
            }), 503
            
    except Exception as e:
        logger.error(f"Personalized genre recommendations error: {e}")
        return jsonify({
...
def get_user_preferences(user_id):
    try:
        ...
        return {
            ...
            'preferred_language': top_languages[0][0] if top_languages else 'english',
...
@app.route('/api/admin/similar-engine/stats', methods=['GET'])
@auth_required
    ...
    try:
        ...
        if similar_engine:
            stats = {
                ...
                'performance_metrics': similar_engine.metrics,
            ...
            return jsonify({
                ...
                'error': 'Similar Content Engine not initialized'
            }), 503
            
    except Exception as e:
        logger.error(f"Error getting similar engine stats: {e}")
        return jsonify({'error': 'Failed to get engine stats'}), 500
    ...
    try:
        ...
        stats = {
            ...
            'services': {
                ...
                'similar_engine': 'enabled' if similar_engine else 'disabled'
            },
            'performance': {
    ...
    try:
        ...
        health_info['performance'] = {
            'optimizations_applied': [
                ...
                'similar_content_engine',
                'advanced_genre_exploration'
            ],
...
if __name__ == '__main__':
    ...
    print("=== Flask app imported by Gunicorn - OPTIMIZED VERSION WITH SIMILAR ENGINE ===")
    ...
    print(f"Similar engine status: {'Initialized' if similar_engine else 'Failed to initialize'}")
    print(f"Performance optimizations: Applied")
    print(f"Unicode fixes: Applied")
    print(f"Cast/Crew support: Fully enabled")
    print(f"Similar content engine: Advanced language-priority enabled")
