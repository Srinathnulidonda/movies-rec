...
class RecommendationEngine:
    ...
    def get_homepage_recommendations():
        """Get homepage recommendations with Telugu first preference"""
        try:
            homepage_data = {
                'telugu': {
                    'trending': [],
            ...
            # Define genres for categorization
            genres = ['Action', 'Drama', 'Comedy', 'Romance', 'Thriller', 'Horror', 'Crime', 'Family']
            
            # Language priority order (Telugu first)
            languages = ['telugu', 'hindi', 'tamil', 'malayalam', 'kannada', 'english']
            
            for language in languages:
                # Get trending movies for this language
                trending = RecommendationEngine.get_trending_by_language(language, limit=20)
                homepage_data[language]['trending'] = trending
                
                # Get all-time hits for this language
                all_time_hits = RecommendationEngine.get_all_time_hits_by_language(language, limit=15)
                homepage_data[language]['all_time_hits'] = all_time_hits
                
                # Get new releases for this language
                new_releases = RecommendationEngine.get_new_releases_by_language(language, limit=15)
                homepage_data[language]['new_releases'] = new_releases
                
                # Get genre-wise content for this language
                for genre in genres:
                    genre_content = RecommendationEngine.get_genre_wise_by_language(language, genre, limit=10)
                    if genre_content:
                        homepage_data[language]['genre_wise'][genre] = genre_content
    ...
    @staticmethod
    def get_trending_by_language(language, limit=20):
        """Get trending movies for specific language"""
        try:
            # Language-specific search terms for better accuracy
            language_search_terms = {
                'telugu': ['telugu movie', 'tollywood', 'telugu cinema', 'telugu film'],
                'hindi': ['hindi movie', 'bollywood', 'hindi cinema', 'hindi film'],
                'tamil': ['tamil movie', 'kollywood', 'tamil cinema', 'tamil film'],
                'malayalam': ['malayalam movie', 'mollywood', 'malayalam cinema', 'malayalam film'],
                'kannada': ['kannada movie', 'sandalwood', 'kannada cinema', 'kannada film'],
                'english': ['hollywood movie', 'english movie', 'american movie', 'english film']
            }
            
            search_terms = language_search_terms.get(language.lower(), [language])
            recommendations = []
            
            # Search for trending content using multiple terms
                ...
                try:
                    ...
                    if trending_data:
                        for item in trending_data.get('results', []):
                            # Filter by language or search term relevance
                            if RecommendationEngine._is_relevant_for_language(item, language):
                    ...
                    # Also search specifically for the language
            ...
            recommendations.sort(key=lambda x: (x.popularity or 0, x.rating or 0), reverse=True)
            return recommendations[:limit]
            
        except Exception as e:
            logger.error(f"Error getting trending for {language}: {e}")
            return []
    
    @staticmethod
    def get_all_time_hits_by_language(language, limit=15):
        """Get all-time hit movies for specific language"""
        try:
            # Define search terms for all-time hits
            language_hit_terms = {
                'telugu': ['best telugu movies', 'classic telugu films', 'top tollywood movies'],
                'hindi': ['best bollywood movies', 'classic hindi films', 'top hindi movies'],
                'tamil': ['best tamil movies', 'classic tamil films', 'top kollywood movies'],
                'malayalam': ['best malayalam movies', 'classic malayalam films', 'top mollywood movies'],
                'kannada': ['best kannada movies', 'classic kannada films', 'top sandalwood movies'],
                'english': ['best hollywood movies', 'classic english films', 'top rated movies']
            }
            
            search_terms = language_hit_terms.get(language.lower(), [f'best {language} movies'])
            recommendations = []
            
            for term in search_terms:
                try:
                    # Search for highly rated content
                    search_results = TMDBService.search_content(term, 'movie')
                    if search_results:
                        for item in search_results.get('results', []):
                            # Filter for high-rated content (7.0+ rating)
                            if (item.get('vote_average', 0) >= 7.0 and 
                                item.get('vote_count', 0) >= 100 and
                                RecommendationEngine._is_relevant_for_language(item, language)):
                    ...
                    logger.warning(f"Error searching for all-time hits with {term}: {e}")
                    continue
            ...
            logger.error(f"Error getting all-time hits for {language}: {e}")
            return []
    
    @staticmethod
    def get_new_releases_by_language(language, limit=15):
        """Get new release movies for specific language"""
        try:
            ...
            language_search_terms = {
                'telugu': [f'telugu movies {current_year}', f'new telugu films {current_year}'],
                'hindi': [f'bollywood movies {current_year}', f'new hindi films {current_year}'],
                'tamil': [f'tamil movies {current_year}', f'new tamil films {current_year}'],
                'malayalam': [f'malayalam movies {current_year}', f'new malayalam films {current_year}'],
                'kannada': [f'kannada movies {current_year}', f'new kannada films {current_year}'],
                'english': [f'new movies {current_year}', f'latest hollywood films {current_year}']
            }
            
            search_terms = language_search_terms.get(language.lower(), [f'new {language} movies {current_year}'])
            recommendations = []
            ...
            for term in search_terms:
                try:
                        ...
                        for item in search_results.get('results', []):
                            ...
                            if release_date:
                                try:
                                    ...
                                    # Filter for recent releases (current year or last year)
                                    if (release_year >= last_year and 
                                        RecommendationEngine._is_relevant_for_language(item, language)):
                    ...
                    logger.warning(f"Error searching for new releases with {term}: {e}")
                    continue
            
            # Sort by release date (newest first)
            recommendations.sort(key=lambda x: x.release_date or datetime.min.date(), reverse=True)
            return recommendations[:limit]
            
        except Exception as e:
            logger.error(f"Error getting new releases for {language}: {e}")
            return []
    
    @staticmethod
    def get_genre_wise_by_language(language, genre, limit=10):
        """Get movies by genre for specific language"""
        try:
            # Combine language and genre for search
            ...
            if search_results:
                for item in search_results.get('results', []):
                    # Check if the movie has the specified genre
                    item_genres = ContentService.map_genre_ids(item.get('genre_ids', []))
                    if (genre in item_genres and 
                        RecommendationEngine._is_relevant_for_language(item, language)):
            ...
            logger.error(f"Error getting {genre} movies for {language}: {e}")
            return []
    
    @staticmethod
    def _is_relevant_for_language(item, language):
        """Check if a movie item is relevant for the specified language"""
        try:
            ...
            # Industry keywords
            industry_keywords = {
                'telugu': ['tollywood', 'telugu', 'hyderabad'],
                'hindi': ['bollywood', 'hindi', 'mumbai'],
                'tamil': ['kollywood', 'tamil', 'chennai'],
                'malayalam': ['mollywood', 'malayalam', 'kerala'],
                'kannada': ['sandalwood', 'kannada', 'bangalore'],
                'english': ['hollywood', 'english', 'american']
            }
            
            target_codes = language_codes.get(language.lower(), [])
            target_keywords = industry_keywords.get(language.lower(), [])
            
            # Check original language
            if original_language in target_codes:
                return True
            
            # Check for keywords in title or overview
            for keyword in target_keywords:
                if (keyword in title or keyword in original_title or keyword in overview):
                    return True
            
            return False
            
        except Exception as e:
            logger.warning(f"Error checking language relevance: {e}")
...
def get_homepage():
    """Get homepage content with Telugu first preference"""
    try:
        ...
        # Process each language (Telugu first)
        for language, content in homepage_data.items():
            formatted_data[language] = {
                'trending': RecommendationEngine._format_content_list(content['trending']),
                'all_time_hits': RecommendationEngine._format_content_list(content['all_time_hits']),
                'new_releases': RecommendationEngine._format_content_list(content['new_releases']),
                'genre_wise': {}
            }
            
            # Format genre-wise content
            for genre, genre_content in content['genre_wise'].items():
                if genre_content:
                    formatted_data[language]['genre_wise'][genre] = RecommendationEngine._format_content_list(genre_content)
        
        return jsonify({
            'homepage_data': formatted_data,
            'language_priority': ['telugu', 'hindi', 'tamil', 'malayalam', 'kannada', 'english'],
            'available_genres': ['Action', 'Drama', 'Comedy', 'Romance', 'Thriller', 'Horror', 'Crime', 'Family'],
            'last_updated': datetime.utcnow().isoformat()
        }), 200
        
    except Exception as e:
        logger.error(f"Homepage error: {e}")
        return jsonify({'error': 'Failed to get homepage content'}), 500

@app.route('/api/content/language/<language>', methods=['GET'])
def get_content_by_language(language):
    """Get all content categories for a specific language"""
    try:
        category = request.args.get('category', 'all')  # trending, all_time_hits, new_releases, genre_wise
        ...
        if category == 'all':
            # Get all categories
            result['trending'] = RecommendationEngine._format_content_list(
                RecommendationEngine.get_trending_by_language(language, limit)
            )
            result['all_time_hits'] = RecommendationEngine._format_content_list(
                RecommendationEngine.get_all_time_hits_by_language(language, limit)
            )
            result['new_releases'] = RecommendationEngine._format_content_list(
                RecommendationEngine.get_new_releases_by_language(language, limit)
            )
            
            # Get genre-wise content
            genres = ['Action', 'Drama', 'Comedy', 'Romance', 'Thriller', 'Horror', 'Crime', 'Family']
            result['genre_wise'] = {}
            for g in genres:
                genre_content = RecommendationEngine.get_genre_wise_by_language(language, g, 10)
                if genre_content:
                    result['genre_wise'][g] = RecommendationEngine._format_content_list(genre_content)
                    
        elif category == 'trending':
            result = RecommendationEngine._format_content_list(
                RecommendationEngine.get_trending_by_language(language, limit)
            )
        ...
        elif category == 'genre_wise' and genre:
            result = RecommendationEngine._format_content_list(
                RecommendationEngine.get_genre_wise_by_language(language, genre, limit)
            )
        ...
        logger.error(f"Language content error: {e}")
        return jsonify({'error': 'Failed to get language content'}), 500

# Add helper method to RecommendationEngine
@staticmethod
def _format_content_list(content_list):
    ...
    try:
        ...
        logger.error(f"Error formatting content list: {e}")
        return []

# Enhanced regional recommendations endpoint
@app.route('/api/recommendations/regional/<language>', methods=['GET'])
def get_enhanced_regional(language):
    """Enhanced regional recommendations with better accuracy"""
    try:
        category = request.args.get('category', 'trending')
        limit = int(request.args.get('limit', 20))
        
        if category == 'trending':
            recommendations = RecommendationEngine.get_trending_by_language(language, limit)
        elif category == 'all_time_hits':
            recommendations = RecommendationEngine.get_all_time_hits_by_language(language, limit)
        elif category == 'new_releases':
            recommendations = RecommendationEngine.get_new_releases_by_language(language, limit)
        else:
            # Default to trending
            recommendations = RecommendationEngine.get_trending_by_language(language, limit)
        
        result = RecommendationEngine._format_content_list(recommendations)
...
# Genre-wise content endpoint
@app.route('/api/content/genre/<genre>', methods=['GET'])
def get_content_by_genre(genre):
    """Get content by genre across all languages"""
    try:
        ...
        if language == 'all':
            # Get genre content for all languages with Telugu first
            languages = ['telugu', 'hindi', 'tamil', 'malayalam', 'kannada', 'english']
            for lang in languages:
                content = RecommendationEngine.get_genre_wise_by_language(lang, genre, limit)
                if content:
                    result[lang] = RecommendationEngine._format_content_list(content)
        else:
            # Get genre content for specific language
            content = RecommendationEngine.get_genre_wise_by_language(language, genre, limit)
            result = RecommendationEngine._format_content_list(content)
        ...
        logger.error(f"Genre content error: {e}")
